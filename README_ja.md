...
- **SwiftUI と UIKit の連携例を探している Swift 開発者** — 5ファイル・合計 535 行で、`UIViewRepresentable` による `UITextView` のラップ、Undo スタックの扱い、`UIPasteControl` の組み込みが一通り読めます。
...
EathWrit はコントリビューションを募集していません。5ファイル・535 行——コーディングエージェントが一度に全部読み切れる分量です。だからこそ、プルリクエストを送るよりも、フォークまたは clone して「自分が欲しい道具」に作り変えてしまうほうが早いはずです。
