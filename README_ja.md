...
2. 24pt で書く             → 16pt〜60pt はステッパーで変更できます
...
- **文字サイズは 16pt〜60pt** — 上部バーのステッパーで 4pt 刻みに変更できます。初期値は 24pt で、現在のサイズは隣にポイント数で表示されます。
- **文字サイズに連動する行間** — 行間はフォントサイズの 35% に設定しているので、60pt にしても 16pt と同じように読みやすさが保たれます。
...
- **作文ツールへのショートカット** — iOS 18.2 以降では、上部バーの杖ボタンで本文全体を選択して Apple の作文ツールを開けます。全選択して長押しメニューを辿る手間が1タップになります。校正・書き直し・要約のどれをさせるかは、そのつど Apple 側の画面で選びます。結果は `UITextView` の標準経路を通るので、Undo で元に戻せます。
- **親指の届く位置にボタンを配置** — よく使う操作（Undo / Redo / Copy & Clear / Paste / Copy）は下部バーに置いています。上部バーは「そのボタンが本文に何をするか」で左右に分けてあり、本文を変えない表示設定（外観・文字サイズ）が左、本文を書き換える・消す操作（作文ツール・Clear）が右です。
...
| 入力面 | 全画面・16〜60pt | 全画面 | 全画面 | 全画面 | 1〜3行のことが多い |
...
- **SwiftUI と UIKit の連携例を探している Swift 開発者** — 5ファイル・合計 537 行で、`UIViewRepresentable` による `UITextView` のラップ、Undo スタックの扱い、`UIPasteControl` の組み込みが一通り読めます。
...
| 位置 | ボタン | 役割 | 補足 |
|---|---|---|---|
| 上部バー | ステッパー `−` / `+` | 文字サイズ変更 | 16〜60pt、4pt 刻み、初期値 24pt |
| 上部バー | ◐ Appearance | System / Light / Dark を選ぶ | 最も左。アイコンが現在の設定を表し、次回起動時も維持されます |
| 上部バー | ✨ Writing Tools | 本文全体を選択して作文ツールを開く | iOS 18.2 以降のみ表示されます。空のときは無効。Undo で戻せます |
...
- **iOS 17.0 以降** — 引数2つの `onChange(of:)` と `Task.sleep(for:)` を使っています。iOS 18.2 以降では上部バーに作文ツールのボタンが1つ増えますが、それ以外は同じです。
...
ステッパーの範囲は 16pt〜60pt で、4pt 刻み、初期値は 24pt です。行間はフォントサイズの 35% として計算されるので、60pt にしても文字が詰まって読みにくくなることはありません。

### AI 機能はありますか？

EathWrit 自体は持っていません。iOS 18.2 以降では、本文全体を選択して Apple の作文ツールを開く杖ボタンがあります。全選択して編集メニューを辿れば同じパネルに行き着くので、EathWrit はその手数を省いているだけです。独自のプロンプトを持たず、どこかに送信もせず、勝手に本文を書き換えることもありません。作文ツールが返した結果は通常の Undo スタックに載るので、Undo で元の文章に戻せます。
...
- **作文ツールは iOS 18.2 以降が必要です** — iOS 17.0〜18.1 では杖ボタン自体が表示されません。Apple Intelligence 非対応端末での挙動は未確認です。
- **自動テストはありません** — テストターゲットは含まれていません。
- **Undo 履歴は保存されません** — 本文は再起動後も残りますが、Undo スタックは残りません。

## フォークして自分のツールにしてください

EathWrit はコントリビューションを募集していません。5ファイル・537 行——コーディングエージェントが一度に全部読み切れる分量です。だからこそ、プルリクエストを送るよりも、フォークまたは clone して「自分が欲しい道具」に作り変えてしまうほうが早いはずです。
